...
public class PizzApp extends javax.swing.JFrame {
    ...
    private void btnRendelActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_btnRendelActionPerformed
        ...
        String uzenet1 = "Választott pizza: ";
        if (cmbValaszthatoPizzak.isShowing()){
            uzenet1 += cmbValaszthatoPizzak.getItemAt(WIDTH);
        }
        else{
            uzenet1 = "Nincs pizza választva!";
        }
        
        String darab = " (" + numDb.getValue() + ")db";
        
        
        String uzenet2 = "Mérete: ";
        if (rdbMeret25.isSelected()){
            uzenet2 += rdbMeret25.getText();
        }
        else if (rdbMeret32.isSelected()){
            uzenet2 += rdbMeret32.getText();
        }
        
        
        String uzenet3 = "Feltétek:\n";
        if (chbSajt.isSelected()){
            uzenet3 += "- " + chbSajt.getText() + "\n";
        }
        if (chbHagyma.isSelected()){
            uzenet3 += "- " + chbHagyma.getText() + "\n";
        }
        if (chbAnanasz.isSelected()){
            uzenet3 += "- " + chbAnanasz.getText() + "\n";
        }
        if (uzenet3.equals("Feltétek:\n")) {
            uzenet3 = "nincs extra feltét választva";
        }
        
        
        String osszesUzenet =  uzenet1 + darab + "\n" + uzenet2 + "\n" + uzenet3;
        txaOsszesito.setText(osszesUzenet);
